...
#########################################################################
#########################################################################
def sdf_extent(model_output, gt):
    '''
       x: batch of input coordinates
       y: usually the output of the trial_soln function
       '''
    ...
    gradient = diff_operators.gradient(pred_sdf, coords)
    # gradient = diff_operators.gradient(pred, coords)

    sdf_constraint = torch.where(gt_sdf != -1, pred_sdf, torch.zeros_like(pred_sdf))
    color_constraint = torch.where(gt_sdf != -1, torch.abs(pred_colors - gt_colors).mean(-1)[..., None], torch.zeros_like(pred_colors[..., :1]))
    inter_constraint = torch.where(gt_sdf != -1, torch.zeros_like(pred_sdf), torch.exp(-1e2 * torch.abs(pred_sdf)))
    # inter_constraint = torch.where(gt_sdf != -1, torch.zeros_like(pred_sdf), torch.exp(1e2 * pred_sdf))
    normal_constraint = torch.where(gt_sdf != -1, 1 - F.cosine_similarity(gradient, gt_normals, dim=-1)[..., None],
                                    torch.zeros_like(gradient[..., :1]))
    grad_constraint = torch.abs(gradient.norm(dim=-1) - 1)
    return {'sdf': torch.abs(sdf_constraint).mean() * 3e3,  # 1e4      # 3e3
            ...
            'color': torch.abs(color_constraint).mean() * 1e2,  
            }


def opacity(model_output, gt):
    '''
       x: batch of input coordinates
       y: usually the output of the trial_soln function
       '''
    gt_opacity = gt['opacities']
    gt_normals = gt['normals']
    gt_colors = gt['colors']
