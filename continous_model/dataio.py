...
##############################################################################
##############################################################################
class PointCloudAir(Dataset):
    def __init__(self, points, on_surface_points, keep_aspect_ratio=True):
        super().__init__()
...
class PointCloudPlus(Dataset):
    def __init__(self, points, on_surface_points, keep_aspect_ratio=True):
        ...
        self.colors = point_cloud[:, 6:]
        self.colors = self.colors * 2. - 1.
    ...
    def __getitem__(self, idx):
        ...
        off_surface_coords = np.random.uniform(-1, 1, size=(off_surface_samples, 3))
        off_surface_normals = np.ones((off_surface_samples, 3)) * -1
        off_surface_colors = np.random.uniform(-1, 1, size=(off_surface_samples, 3))
...
class ViewPointPostProcess(Dataset):
    ...
    def find_bound(self, points, keep_aspect_ratio):
        self.mean = torch.mean(points, dim=0, keepdim=True)
        points -= self.mean
        if keep_aspect_ratio:
            lower_bound = torch.min(points)
            upper_bound = torch.max(points)
        else:
            lower_bound = torch.min(points, dim=0, keepdim=True).values
    ...
    def normalize_points(self, points):
        points -= self.mean
        points = (points - self.lower_bound) / (self.upper_bound - self.lower_bound)
        points = points * 2. - 1.
        return points


    def depth2points(self, depth_map, viewpoint_camera):
    ...
    def transform2w(self, points, normals, viewpoint_cam):
        w2c = viewpoint_cam.world_view_transform.transpose(0, 1)
        c2w = torch.inverse(w2c)
        points = torch.cat([points, torch.ones(points.shape[0], 1, device=points.device)], dim=1)
        points = torch.matmul(c2w, points.t()).t()
        points = points[:, :3]
        normals = torch.matmul(c2w[:3, :3], normals.t()).t()
    ...
    def propogate_data(self, depth_map, normal_map, color_image, opacities, viewpoint_cam, voxelize=True):
        ...
        opacity = opacities.permute(1, 2, 0).view(-1, 1)[mask]
        points, normals = self.transform2w(points, normals, viewpoint_cam)
        ...
        if points_size > self.batch_size:
            rand_inds = torch.randperm(points_size)[:self.batch_size]
            points = points[rand_inds]
            normals = normals[rand_inds]
            colors = colors[rand_inds]
            opacity = opacity[rand_inds]
        
        off_surface_batch = self.batch_size
        total_samples = self.batch_size + off_surface_batch

        off_surface_points = torch.rand([off_surface_batch, 3], device=points.device) * 2. - 1.
        off_surface_normals = torch.ones([off_surface_batch, 3], device=points.device) * -1
        # off_surface_color = torch.rand([off_surface_batch, 3], device=points.device) * 2. - 1
        off_surface_color = torch.ones([off_surface_batch, 3], device=points.device) * -1
        off_surface_opacity = torch.rand([off_surface_batch, 1], device=points.device) * -1

        sdf = torch.zeros([total_samples, 1], device=points.device)
        sdf[self.batch_size:, :] = -1

        coords = torch.cat([points, off_surface_points], dim=0).float()
        normals = torch.cat([normals, off_surface_normals], dim=0).float()
        colors = torch.cat([colors, off_surface_color], dim=0).float()
        opacity = torch.cat([opacity, off_surface_opacity], dim=0).float()

        return {'coords': coords}, {'sdf': sdf, 'normals': normals, 'colors': colors, 'opacities': opacity}
    

    def propogate_opacity(self, depth, normal, image, opacities, viewpoint_cam):
        points, mask = self.depth2points(depth, viewpoint_cam)
        normals = normal.permute(1, 2, 0).view(-1, 3)[mask]
        colors = image.permute(1, 2, 0).view(-1, 3)[mask]
        opacity = opacities.permute(1, 2, 0).view(-1, 1)[mask]
        points, normals = self.transform2w(points, normals, viewpoint_cam)
        ...
        return {'coords': coords}, {'opacities': opacity, 'normals': normals, 'colors': colors}
##############################################################################
##############################################################################
##############################################################################
