...
**********************************************************************
"""
def training(dataset, opt, pipe, dataset_name, saving_iterations, debug_from, wandb=None, logger=None, ply_path=None, testing_freq=1000, debug=True, render_with_gsplat=False, train_with_depth=True, train_with_normal=False, absgrad=True, train_with_scaledown=True, antialiasing=False):
    first_iter = 0
    ...
    ######################################################################
    depth_l1_weight = get_expon_lr_func(0.1, 0.01, max_steps=opt.iterations)
    normal_l1_weight = get_expon_lr_func(0.1, 0.01, max_steps=opt.iterations)
    ######################################################################
    ...
    while iteration <= opt.iterations:
        ...
        for idx in range(num_blocks_per_gpu):
            ...
            for cur_iter in range(iteration, end_iter):
                ...
                ###############################################################
                # offset_scale = gaussians.update_offset_scale(render_pkg["xyz"], viewpoint_cam, voxel_visible_mask, offset_selection_mask, visibility_filter)
                # Ll1 = l1_loss(image, gt_image)
                # ssim_loss = (1.0 - ssim(image, gt_image))
                # offset_scale_mask = offset_scale.squeeze() < 1000
                # # offset_scale_middle = scaling.topk(2, dim=1).values[:, -1]
                # offset_scale_middle = scaling.min(dim=1).values
                # offset_scale_middle_mask = offset_scale_middle < offset_scale.squeeze()
                ...
                # scaling_max_reg = scaling.max(dim=1).values[~combine_mask].mean()
                # # scaling_reg = scaling.prod(dim=1)[~combine_mask].mean()
                # # loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss + 0.01 * scaling_reg + 0.2 * scaling_min_reg
                # scaling_reg = scaling.sum(dim=1)[~combine_mask].mean()
                # loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss + 0.01 * scaling_reg + 0.1 * (scaling_min_reg + scaling_max_reg)
                ###############################################################
                ...
                if train_with_depth:
                    ...
                    Ll1_depth = torch.abs(depth - gt_depth).mean() * depth_l1_weight(cur_iter)
                    # Ll1_depth = torch.abs(depth - gt_depth).mean() * (1 - opt.lambda_dssim) * 0.1
                    loss += Ll1_depth
                
                if train_with_normal:
                    Ll1_normal = torch.abs(normal - gt_normal).mean() * normal_l1_weight(cur_iter)
                    # Ll1_normal = torch.abs(normal - gt_normal).mean() * (1 - opt.lambda_dssim) * 0.1
                    loss += Ll1_normal
                ###############################################################
                ###############################################################
