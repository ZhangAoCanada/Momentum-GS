...
class GaussianModel:
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            if  'mlp' in group['name'] or \
                ...
                'appearance_embeddings' in group['name'] or \
                'appearance_network' in group['name']:
                continue
    ...
    def anchor_growing(self, grads, threshold, offset_mask):
        ...
        for i in range(self.update_depth):
            ...
            all_xyz = self.get_anchor.unsqueeze(dim=1) + self._offset * self.get_scaling[:,:3].unsqueeze(dim=1) # [n, n_offsets, 3]
            ...
            selected_grid_coords_unique_np = selected_grid_coords.cpu().numpy()
            selected_grid_coords_unique_np, index_array, inverse_indices = np.unique(selected_grid_coords_unique_np, return_index=True, return_inverse=True, axis=0)

            selected_grid_coords_unique = torch.tensor(selected_grid_coords_unique_np).to(selected_grid_coords.device)
            inverse_indices = torch.tensor(inverse_indices).to(selected_grid_coords.device)
            ...
            if candidate_anchor.shape[0] > 0:
                ...
                # Using chunk to reduce memory usage
                chunk_size = 1000000

                start_idx = 0
                while start_idx < self._anchor_feat.shape[0]:
                    end_idx = start_idx + chunk_size
                    chunk_candidate_mask = candidate_mask[start_idx*self.n_offsets:end_idx*self.n_offsets]
                    new_feat_chunk = self._anchor_feat[start_idx:end_idx].unsqueeze(dim=1).repeat([1, self.n_offsets, 1]).view([-1, self.feat_dim])[chunk_candidate_mask]
                    new_feat = torch.cat([new_feat, new_feat_chunk], dim=0) if start_idx > 0 else new_feat_chunk
                    start_idx = end_idx

                del new_feat_chunk
                del chunk_candidate_mask
    ...
    def load_mlp_checkpoints(self, path, mode = 'split'):
        if mode == 'split':
            self.mlp_opacity = torch.jit.load(os.path.join(path, 'opacity_mlp.pt'), map_location='cuda').cuda()
            self.mlp_cov = torch.jit.load(os.path.join(path, 'cov_mlp.pt'), map_location='cuda').cuda()
            self.mlp_color = torch.jit.load(os.path.join(path, 'color_mlp.pt'), map_location='cuda').cuda()
            if self.use_feat_bank:
                self.mlp_feature_bank = torch.jit.load(os.path.join(path, 'feature_bank_mlp.pt'), map_location='cuda').cuda()
            if self.appearance_dim > 0:
                self.embedding_appearance = torch.jit.load(os.path.join(path, 'embedding_appearance.pt'), map_location='cuda').cuda()
        elif mode == 'unite':
            checkpoint = torch.load(os.path.join(path, 'checkpoints.pth'), map_location='cuda')
            self.mlp_opacity.load_state_dict(checkpoint['opacity_mlp'], map_location='cuda')
            self.mlp_cov.load_state_dict(checkpoint['cov_mlp'], map_location='cuda')
            self.mlp_color.load_state_dict(checkpoint['color_mlp'], map_location='cuda')
            if self.use_feat_bank:
                self.mlp_feature_bank.load_state_dict(checkpoint['feature_bank_mlp'], map_location='cuda')
            if self.appearance_dim > 0:
                self.embedding_appearance.load_state_dict(checkpoint['appearance'], map_location='cuda')
        else:
            raise NotImplementedError

    def freeze_mlp(self):
        self.freeze_all_mlp = True
