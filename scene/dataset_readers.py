...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, woimage=False, heavyimage=True):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if woimage:
            ...
            image4_path = os.path.join(images_folder.replace(image_folder_name, f"{image_folder_name}_4"), os.path.basename(extr.name).replace("jpg", "png").replace("JPG", "png"))
            if os.path.exists(image4_path):
                image_4 = Image.open(image4_path)
            else:
                image_4 = None

        image6_path = os.path.join(images_folder.replace(image_folder_name, f"{image_folder_name}_6"), os.path.basename(extr.name).replace("jpg", "png").replace("JPG", "png"))
...
def readColmapSceneInfo(path, images, eval, lod, llffhold=8, meganerf_partition=False, train_val_partition=False, train_test_partition=False, partition=None, woimage=False, heavyimage=True):
    ...
    if woimage:
        ...
        try:
            ...
            if train_val_partition:
                val_path = path.replace("train", "val")
                val_cameras_extrinsic_file = os.path.join(val_path, "sparse/0", "images.bin")
                val_cameras_intrinsic_file = os.path.join(val_path, "sparse/0", "cameras.bin")
                val_cam_extrinsics = read_extrinsics_binary(val_cameras_extrinsic_file)
                val_cam_intrinsics = read_intrinsics_binary(val_cameras_intrinsic_file)
            elif train_test_partition:
                val_path = path.replace("train", "test")
                val_cameras_extrinsic_file = os.path.join(val_path, "sparse/0", "images.bin")
                val_cameras_intrinsic_file = os.path.join(val_path, "sparse/0", "cameras.bin")
                val_cam_extrinsics = read_extrinsics_binary(val_cameras_extrinsic_file)
                val_cam_intrinsics = read_intrinsics_binary(val_cameras_intrinsic_file)
            ...
            if train_val_partition:
                val_path = path.replace("train", "val")
                val_cameras_extrinsic_file = os.path.join(val_path, "sparse/0", "images.txt")
                val_cameras_intrinsic_file = os.path.join(val_path, "sparse/0", "cameras.txt")
                val_cam_extrinsics = read_extrinsics_text(val_cameras_extrinsic_file)
    ...
    if train_val_partition or train_test_partition:
        ...
        val_cam_infos = sorted(val_cam_infos_unsorted.copy(), key = lambda x : x.image_name)
    ...
    if eval:
        if lod>0:
            ...
            print(f'using llffhold, using eval')
            train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
            test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
    elif meganerf_partition:
        print('### using meganerf_partition')
        with open(os.path.join(path, "meganerf_test.yaml"), "r") as file:
            test_cam_list = yaml.safe_load(file)
        train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx not in test_cam_list]
        test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx in test_cam_list]
    elif train_val_partition or train_test_partition:
        if train_val_partition:
            print('### using train_val_partition')
        else:
            print('### using train_test_partition')
