...
class GaussianInterpolation(nn.Module):
    def __init__(self, args, scene, opt, pipe, trained_gaussians, tb_writer, depth_threshold=0.1, color_threshold=0.1, voxel_stride_portion=0.5, interpolate_interval=10, train_iterations=500): # interval 10 iter 500
        super().__init__()
    ...
    def forward(self, debug=True, resolution_scaling=2, scaledown_ratio=1.):
        ...
        # resolution_scaleup = resolution_scaling is not None and resolution_scaling > 1.
        ...
        for fid in range(num_frames):
            ...
            with torch.no_grad():
                ...
                render_pkg_pretrained = render_gsplat(viewpoint_cam, self.trained_gaussians, self.pipe, background, visible_mask=voxel_visible_mask_trained, retain_grad=False, absgrad=False, resolution_scaling_factor=resolution_scaling)
            ...
            for iteration in range(self.iterations):
                ...
                if debug and fid % 10 == 0 and iteration == self.iterations - 1:
                    image_residual = torch.abs(image - rendered_image)
                    depth_residual = torch.abs(depth - rendered_depth)
                    rgbs = torch.cat([gt_image, rendered_image, image, image_residual, torch.abs(gt_image - image)], dim=1)
                    depths = torch.cat([gt_depth, rendered_depth, depth, depth_residual, torch.abs(gt_depth - depth)], dim=1)
                ...
                iteration += 1
            print(f"[DEBUG] {fid}/{num_frames}: anchors: {self.trained_gaussians.get_anchor.shape[0]}, interpolation: {self.gaussians.get_anchor.shape[0]}, stride: {self.voxel_stride}")
