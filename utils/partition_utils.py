# Copy from CityGaussian, thanks for their contribution.
import torch
import numpy as np
...
def contract_to_unisphere(
    ...
    aabb_min, aabb_max = torch.split(aabb, 3, dim=-1) #  [-50., -100., -135.], [ 50., 300.,  -5.]
    x = (x - aabb_min) / (aabb_max - aabb_min) # e.g. [  29.8492, 31.2157, -101.3596] -> [0.7985, 0.3280, 0.2588]
    x = x * 2 - 1  # aabb is at [-1, 1] # e.g. [0.7985, 0.3280, 0.2588] -> [ 0.5970, -0.3439, -0.4825], x大约是(-5.10, 4.78)
    mag = torch.linalg.norm(x, ord=ord, dim=-1, keepdim=True) # 最长的那个分量(x or y or z)的绝对值
